...
export class EmpresaUsuariosComponent implements OnInit {
  ...
  loadUsers() {
    ...
    this.http
      .get<any[]>(`https://tcc-main.up.railway.app/user?companyId=${this.selectedCompany.id}`)
      .subscribe({
        next: (data) => {
          this.users = data || [];
        },
        error: (err) => {
          this.usersError = err?.error?.message || "Erro ao carregar usuários";
        },
        complete: () => {
  ...
  toggleAdmin(u: any) {
    ...
    this.http.put(`https://tcc-main.up.railway.app/user/${u.id}/role`, { role: targetRole }).subscribe({
      next: (resp: any) => {
        u.role = resp?.user?.role || targetRole;
      },
  ...
  resetAdminForm() {
    this.adminForm = { first_Name: "", last_Name: "", email: "", password: "" };
  ...
  submitCreateAdmin() {
    ...
    this.http.post("https://tcc-main.up.railway.app/user/register-access", payload).subscribe({
      next: (resp: any) => {
        this.adminSuccess = "Administrador criado";
        this.loadUsers();
      },
      error: (err) => {
        this.adminError = err?.error?.details || err?.error?.message || "Erro";
      },
    });
  }
