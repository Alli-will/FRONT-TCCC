...
export class PerguntaNovaComponent {
  ...
  salvar() {
    if (!this.texto.trim()) {
      this.showBanner("Informe o texto da pergunta.", "erro");
      return;
    }
    this.questions
      .create({
        texto: this.texto.trim(),
        descricaoBusca: this.descricaoBusca?.trim(),
        modalidade: this.modalidade,
        tipoResposta: this.tipoResposta,
      })
      .subscribe({
        next: () => {
          this.showBanner("Pergunta cadastrada com sucesso.", "sucesso");
          setTimeout(() => history.back(), 600);
        },
        error: (e) => {
          this.showBanner("Falha ao cadastrar pergunta.", "erro");
          console.error(e);
        },
      });
  }
