...
@Component({
      ...
      <form (ngSubmit)="submit()" #f="ngForm" novalidate>
        ...
        <div class="linha-form" [class.invalido]="form.password && invalido() && tentativa">
          ...
          <div class="pw-hints" *ngIf="form.password">
            <span *ngFor="let item of missingPasswordCriteria()" class="hint" [class.ok]="false">{{
              item
            }}</span>
            <span *ngIf="missingPasswordCriteria().length === 0" class="ok-msg"
              >Senha atende aos requisitos.</span
            >
          </div>
        </div>
        <div class="acoes">
  ...
  styles: [
    `
      ...
      .cadastro-colaborador-container {
        max-width: 640px;
        margin: 24px auto;
        background: #ffffff;
      ...
      .top-bar .btn-voltar {
        ...
        font-size: 0.85rem;
        box-shadow: 0 4px 14px rgba(56, 182, 165, 0.35);
      }
      .top-bar .btn-voltar:active {
      ...
      select:focus {
        border-color: #38b6a5;
        background: #fff;
        box-shadow: 0 0 0 3px rgba(56, 182, 165, 0.15);
      }
      .linha-form.invalido input,
      .linha-form.invalido select {
        border-color: #e53935;
        background: #fff5f5;
      }
      .pw-hints {
      ...
      .btn-primario {
        ...
        align-items: center;
        gap: 0.5rem;
        background: linear-gradient(135deg, #38b6a5, #2d998b);
        color: #fff;
        border: none;
        ...
        border-radius: 14px;
        cursor: pointer;
        box-shadow: 0 6px 18px rgba(56, 182, 165, 0.35);
        position: relative;
        overflow: hidden;
        transition:
          transform 0.25s,
      ...
      .btn-primario:hover {
        ...
        box-shadow: 0 10px 22px -4px rgba(56, 182, 165, 0.45);
      }
      .btn-primario:active {
        transform: translateY(0);
        box-shadow: 0 6px 14px -4px rgba(56, 182, 165, 0.4);
      }
      .btn-primario[disabled] {
        opacity: 0.55;
        cursor: not-allowed;
        transform: none;
        box-shadow: 0 4px 12px rgba(56, 182, 165, 0.25);
      }
      .sucesso {
        color: #1b5e20;
        background: #e8f5e9;
