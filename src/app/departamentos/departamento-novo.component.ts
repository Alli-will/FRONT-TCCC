  ...
  styles: [
    `
      ...
      .actions .btn-sec {
        ...
        color: #fff;
        box-shadow: 0 4px 12px rgba(56, 182, 165, 0.3);
      }
      .actions .btn-sec:hover:not(:disabled) {
        filter: brightness(1.05);
      }
      .actions .btn-primario {
        min-width: 130px;
      }
      .loader {
        width: 18px;
...
export class DepartamentoNovoComponent {
  ...
  criar() {
    ...
    this.http.post("https://tcc-main.up.railway.app/departments", { name: this.name }).subscribe({
      next: () => {
        this.success = "Departamento criado";
        this.name = "";
      },
      error: (err) => {
        this.error = err?.error?.message || "Erro ao criar";
      },
      complete: () => {
        this.loading = false;
      },
