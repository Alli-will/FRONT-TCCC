...
@Component({
  ...
  <div class="reset-wrapper" *ngIf="!finalizado">
      ...
      <form (ngSubmit)="salvar()">
        ...
        <input [(ngModel)]="senha" name="senha" type="password" required minlength="6" placeholder="••••••"/>
        <label>Confirmar Senha</label>
        <input [(ngModel)]="confirm" name="confirm" type="password" required minlength="6" placeholder="••••••"/>
        <button class="btn-primario" [disabled]="loading">{{ loading ? 'Salvando...' : 'Salvar' }}</button>
      </form>
  ...
  `,
  styles:[`
  .page-reset { min-height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:flex-start; padding-top:4rem; background:linear-gradient(135deg,#f4fafc,#eef6ff); }
    .logo-top { width:240px; max-width:70%; height:auto; margin:0 0 1.8rem; filter: drop-shadow(0 6px 14px rgba(0,0,0,.12)); }
  .reset-wrapper { width:100%; max-width:400px; background:#fffffffa; padding:2.4rem 2.1rem 2.2rem; border-radius:1.25rem; box-shadow:0 8px 30px -6px rgba(0,0,0,.14); display:flex; flex-direction:column; gap:1.1rem; border:1px solid #e3f3f4; backdrop-filter:blur(3px); }
  /* back button removido */
    h2 { margin:0 0 .5rem; font-size:1.4rem; font-weight:700; }
    form { display:flex; flex-direction:column; gap:.8rem; }
    label { font-size:.7rem; font-weight:600; letter-spacing:.5px; text-transform:uppercase; color:#2d5b66; }
  ...
  .sucesso { background:#e6fcf5; color:#166c58; border:1px solid #b6f2e1; }
    ...
    .link-login { text-decoration:none; display:inline-block; padding:.7rem 1rem; margin-top:.5rem; border-radius:.8rem; }
    @media (max-width:650px){ .page-reset { padding-top:3rem; } .logo-top { width:200px; margin-bottom:1.4rem; } .reset-wrapper { padding:2rem 1.6rem 1.8rem; } }
  `]
})
export class DefinirNovaSenhaComponent {
  private route = inject(ActivatedRoute);
  private auth = inject(AuthService);
  senha=''; confirm=''; erro=''; loading=false; finalizado=false;
  token = this.route.snapshot.paramMap.get('token') || '';
  salvar() {
    if (this.senha !== this.confirm) { this.erro='Senhas não conferem.'; return; }
    if (this.loading) return; this.erro=''; this.loading=true;
    this.auth.resetPassword(this.token, this.senha).subscribe({
      next: () => { this.finalizado=true; this.loading=false; },
      error: () => { this.erro='Token inválido ou expirado.'; this.loading=false; }
    });
  }
}
