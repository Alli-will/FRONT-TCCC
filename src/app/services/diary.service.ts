...
export class DiaryService {
  private apiUrl = 'http://https://tcc-main.up.railway.app/diary';
  private apiAllUrl = 'http://https://tcc-main.up.railway.app/diary/all';
  ...
  hasEntryToday(token: string): Observable<any> {
    const headers = this.getAuthHeaders(token);
    return this.http.get('http://https://tcc-main.up.railway.app/diary/has-entry-today', { headers });
  }
  ...
  getUserEss(token: string): Observable<any> {
    const headers = this.getAuthHeaders(token);
    return this.http.get('http://https://tcc-main.up.railway.app/dashboard/ess', { headers });
  }
