...
export class PerfilComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.userService.getCurrentUser().subscribe({
      ...
      error: () => {
        this.carregando = false;
        this.erro = "Falha ao carregar perfil";
      },
    });
  }
  ...
  onFileChosen(ev: Event) {
    ...
    if (!file.type.startsWith("image/")) {
      this.uploadErro = "Arquivo precisa ser imagem";
      return;
    }
    if (file.size > 2 * 1024 * 1024) {
      this.uploadErro = "Máx 2MB";
      return;
    }
  ...
  private carregarAvatar(force?: boolean) {
    ...
    const apiBase = window.location.hostname.includes("localhost")
      ...
      : "https://tcc-main.up.railway.app";
    this.loading.block();
    fetch(`${apiBase}/user/me/avatar/meta?ts=${this.timestamp}`, {
      ...
      .catch(async () => {
        // Fallback base64
        try {
          const r = await fetch(`${apiBase}/user/me/avatar/base64?ts=${this.timestamp}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          if (r.ok) {
        ...
        try {
          const r2 = await fetch(`${apiBase}/user/me/avatar?ts=${this.timestamp}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          if (r2.ok) {
