...
export class EmpresaUsuariosComponent implements OnInit {
  ...
  loadCompany(id: number) {
    this.http.get(`https://tcc-main.up.railway.appcompanies/${id}`).subscribe({
  ...
  if (!this.selectedCompany) return;
    ...
    this.http.get<any[]>(`https://tcc-main.up.railway.appuser?companyId=${this.selectedCompany.id}`).subscribe({
  ...
  toggleAdmin(u: any) {
    ...
    this.http.put(`https://tcc-main.up.railway.appuser/${u.id}/role`, { role: targetRole }).subscribe({
  ...
  submitCreateAdmin() {
    ...
    this.http.post('https://tcc-main.up.railway.appuser/register-access', payload).subscribe({
